...
function SavedProperty() {
    ...
    for (const property of shortlisted) {
      try {
        const response = await fetch(
          `https://apex.oracle.com/pls/apex/boyce/savedProperty/getPropertyInfo?asking_type=${asking_type}&property_id=${property}`
        );
        if (!response.ok) {
        ...
        if (!localRetrivedPropertyID.includes(result.items[0].property_id)) {
          ...
          setSavedPropertyInfo((prevData) => {
            ...
            if (
              !prevData.some(
                (item) => item.property_id === result.items[0].property_id
              )
            ) {
  ...
  return (
    <>
      <div className="saved-property-section-container">
        <h1>My saved properties ({asking_type})</h1>
        <div className="saved-property-container">
          {loading ? (
            <SkeletonSales />
          ) : (
            <div className="ads-container">
              {savedPropertyInfo.map((item) => (
                <div key={item.property_id} className="ad-item">
                  <Ads
                    propertyInfo={item}
